...
## Overview
Localsetup v2 is deployed at `_localsetup/`. The framework is repo-local and mobile, but mutable user/project state should live in stable repo-level paths outside `_localsetup/`. Engine = _localsetup/. Use Git hashes for PRDs/specs (see [GIT_TRACEABILITY.md](../../docs/GIT_TRACEABILITY.md)).

## Invariants
- **Engine/repo separation:** Never commit repo-local secrets or PII. Use _localsetup/lib/data_paths.sh (or equivalent) for path resolution. Framework lives at _localsetup/; upgrades replace that folder. Never store mutable memory, reminders, backlog, or temporary notes under `_localsetup/`.
