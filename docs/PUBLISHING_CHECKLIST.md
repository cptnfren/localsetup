...
- [ ] **Framework README**  - `_localsetup/README.md` describes engine, tools, skills, and docs; links to root where needed.
...
- [ ] **Issue/Discussion links**  - README and `_localsetup/README.md` use the correct repo owner in GitHub URLs.
- [ ] **Deploy artifacts**  - nested `_localsetup/_localsetup/` is not tracked in source repos (it may be created in client repos by install/deploy).

## Version and release

- [ ] **VERSION**  - Set to the intended release version (e.g. 2.0.0).
- [ ] **Version display**  - README and `_localsetup/README.md` show the same version; `_localsetup/docs` front matter `version: X.Y` matches if used.
