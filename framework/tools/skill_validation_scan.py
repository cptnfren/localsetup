...
PATTERN_FILE_RAW_URL = "https://raw.githubusercontent.com/cptnfren/localsetup/main/framework/docs/SKILL_VALIDATION_PATTERNS.yaml"
...
def resolve_pattern_file_path(pattern_file: Path | None, scan_root: Path) -> Path:
    """Resolve path: explicit > _localsetup/docs/ (under scan_root) > framework/docs/."""
    if pattern_file:
        p = Path(pattern_file)
        if p.is_absolute():
            return p
    ...
    if localsetup.exists():
        return localsetup
    # Fallback: framework from source (script lives in framework/tools/)
